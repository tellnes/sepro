...
function SeaportRouter(options) {
  ...
  ports.on('assume', onService)
  ports.on('allocate', onService)

  ports.query(function(services) {
    Object.keys(services).forEach(function(role) {
      services[role].forEach(onService)
    })
  })
