...
Router.prototype.handle = function(req, resOrHead, next) {
    ...
    if (location.length > 1) {
      if (req.sticky) {
        for(var i = 0, len = location.length; i < len; i++) {
          if (!location[i].dead && this._getKey(location[i]) == req.sticky) {
            index = i
            break
          }
        }
      ...
      if (index == -1) {
        ...
        while(location.length) {
          index = Math.floor(Math.random() * location.length)
          if (location[index].dead) location.splice(1, index)
        }
      }
  ...
  if (req.upgrade) {
    debug('proxy upgrade for %s to %s:%s', req.headers.host, location.host, location.port)
    this.proxyWebSocketRequest(req, req.socket, resOrHead, location)

  } else {
    debug('proxy request for %s to %s:%s', req.headers.host, location.host, location.port)
...
Router.prototype.getNodes = function(nodes) {
